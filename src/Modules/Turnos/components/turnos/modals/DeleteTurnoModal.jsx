...
export const DeleteTurnoModal = ({
  ...
  const { getTurnos, deleteTurno } = useContext(TurnosContext);
    ...
    return (
      <div className="m-5">
        <h2 className="text-center text-3xl font-bold text-gray-800">
          {`Eliminar Turno: ${row ? row.date + " , " + row.time : ""}`}
        </h2>
      </div>
    );
  };
  const BodyModal = () => {
    return (
      <div>
        ...
        <div>
          <p className="text-xl">
            {`Va a eliminar el turno: ${row.date} , ${row.time}`}
          </p>
